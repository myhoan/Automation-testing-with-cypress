export const PracticeFormPage = {
    registerStudent(firstName,lastName,email,gender,phoneNumber,dob,subject,hobbies,picture,currentAddress,state,city){
        const hobbiesString = hobbies;

    cy.get('#firstName').type(firstName)
    cy.get('#lastName').type(lastName)
    cy.get('#userEmail').clear().type(email)
    const validGender = cy.get("label[for='gender-radio-2']").should('have.text',`${gender}`)
    validGender.click()
    ...
    const validHobbie = cy.get("label[for='hobbies-checkbox-1']").should("have.text",`${hobbies}`)
    validHobbie.click()
